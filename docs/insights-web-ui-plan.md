...
- work within a 2-CPU/3.7 GB machine, complete a 73-video build in seconds or low minutes, and remain comfortably below 100 MB (GH Pages limit is 1 GB; budget is a self-imposed sanity gate);
...
Pass means every local `href`, `src`, and referenced fragment resolves; all analyzed items have pages; the only allowed outbound content URLs are YouTube watch URLs derived from `source_uri`; no absolute host path, `file:` URL, source artifact pathname, or disallowed external URL occurs; and total generated size is under 100 MB.
